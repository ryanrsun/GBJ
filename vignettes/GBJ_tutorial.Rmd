...
1. A unified, parametric, and theoretically justified approach to combining sets of correlated test statistics.  This may provide additional power over more ad-hoc or nonparametric procedures that use count or rank procedures.
2. Procedure is straightfoward and direct. No tuning parameters to fit or data filtering necessary for sets that are too small.  It is reasonable to apply GBJ on sets ranging in size from 2 to ~950 factors 
...
4.  Output is a p-value, which can be interpreted both as a stand-alone measure or as a relative ranking, for example to rank pathways in pathway analysis.
...
The function `calc_score_stats()` can be used to calculate score statistics for each of the 50 SNPs.  Under the null, these statistics will have an asymptotic N(0,1) distribution.  If our outcome was continuous and we assumed a linear regression model, then we could still use `calc_score_stats()` with `model_type='linear'`.  Otherwise, if we wish to assume a more unique model, the user will have to calculate their own test statistics (should be asymptotically N(0,1) under the null) for each SNP.

```{r}
null_mod <- glm(cancer_status~age+gender, family=binomial(link="logit"))
log_reg_stats <- calc_score_stats(null_mod, genotype_data, model_type="logistic")
log_reg_stats$test_stats
log_reg_stats$cor_mat[1:5,1:5]
```

At this point we can extract the vector of pairwise correlations using the function `upper.tri()` and then apply GBJ or any of the other set-based methods provided by this package. (If starting from summary-level data, begin at this step - simple, right?).  

```{r}
cor_vec <- log_reg_stats$cor_mat[upper.tri(log_reg_stats$cor_mat)]
score_stats = log_reg_stats$test_stats
GBJ(test_stats=score_stats, pairwise_cors=cor_vec)
GHC(test_stats=score_stats, pairwise_cors=cor_vec)
HC(test_stats=score_stats, pairwise_cors=cor_vec)
BJ(test_stats=score_stats, pairwise_cors=cor_vec)
minP(test_stats=score_stats, pairwise_cors=cor_vec)
```

## Some advice for special cases

- Note that in the above example we know each of our factors (SNPs) are independent, so another option would be to just input a correlation vector where all the elements are zero.  This should not drastically change the results since our estimated correlations are so close to 0 anyway.  In general, if we know the correlations, then it is optional to estimate them.

- To estimate the correlation vector when starting with genetic consortium summary statistics, we have found that a close approximation is given by the correlations of the SNPs themselves.  Specifically, one can navigate to the 1000 Genomes website, download the genotype data at each of the SNPs being tested by GBJ (making sure to only use subjects from an ethnicity close to the one studied by the consortium), and then take the sample correlations between the SNPs.  These sample correlations can then be used as the correlation vector.
